...
import { formatDate, getPaymentMethodLabel } from './helpers';
...
export const exportAsCSV = async (expenses: Expense[], filename: string): Promise<void> => {
  // CSV column headers
  const headers = ['Date', 'Amount', 'Category', 'Payment Method', 'Notes', 'Tags', 'Currency'];
...
export const exportAsExcel = async (expenses: Expense[], filename: string): Promise<void> => {
  ...
  xml += '      <Row>\n';
  ['Date', 'Amount', 'Category', 'Payment Method', 'Notes', 'Tags', 'Currency'].forEach(h => {
  ...
  expenses.forEach(e => {
    ...
    xml += `        <Cell><Data ss:Type="String">${getPaymentMethodLabel(e.paymentMethod)}</Data></Cell>\n`;
...
<table>
<tr><th>#</th><th>Date</th><th>Category</th><th>Amount</th><th>Payment</th><th>Notes</th></tr>`;
  ...
  expenses.forEach((e, i) => {
    html += `<tr>
      ...
      <td>${getPaymentMethodLabel(e.paymentMethod)}</td>
      <td>${e.notes || '-'}</td>
    </tr>`;
  });

  // Total row at the bottom of the table
  html += `<tr class="total-row"><td colspan="3">Total</td><td>${currencySymbol}${totalAmount.toFixed(2)}</td><td colspan="2"></td></tr>`;
  html += `</table>
